...
export default function BlackHoleUniverse() {
  // 1. ЗУРГУУДЫН ЖАГСААЛТ (Энд public доторх зургуудынхаа замыг бичээрэй)
  const images = [
    "/enigma1.jpg",
    "/enigma2.jpg",
    "/enigma3.jpg",
  ...
  // 2. 6 СЕКУНД ТУТАМД ЗУРГИЙГ СОЛИХ ЛОГИК
  useEffect(() => {
  ...
  return (
    <div className="relative w-full h-full bg-black overflow-hidden flex items-center justify-center">
      ...
      {/* ЗУРГИЙН СЛАЙДЕР ХЭСЭГ */}
      <div className="absolute inset-0 z-0">
        <AnimatePresence mode="wait">
          <motion.img
            key={index} // Key өөрчлөгдөхөд анимаци ажиллана
            src={images[index]}
            initial={{ opacity: 0, scale: 1.1 }}
            animate={{ opacity: 0.6, scale: 1 }}
            exit={{ opacity: 0, scale: 1.05 }}
            transition={{ duration: 2.5, ease: "easeInOut" }} // Маш зөөлөн шилжилт
            className="w-full h-full object-cover grayscale-[0.2] contrast-125"
            alt="Black Hole Scene"
          />
        </AnimatePresence>
        
        {/* Зургийн дээрх харанхуйлах давхарга (Gradient Overlay) */}
        <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-black/90 z-10" />
      </div>

      {/* ТЕКСТҮҮД (Өмнөх сүрлэг текстүүд хэвээрээ) */}
